...
class MBPOAgent(BaseAgent):
    ...
    def collect_model_trajectory(self, rollout_length=1):
        # TODO (Q6): Collect a trajectory of rollout_length from the learned dynamics model. Start from a state sampled from the replay buffer.
        ...
        for _ in range(rollout_length):
            ...
            # determine the next observation by averaging the prediction of all the dynamics models in the ensemble
            next_ob = np.array([model.get_prediction(ob, ac, self.mb_agent.data_statistics) for model in self.mb_agent.dyn_models]).mean(axis=0)

            # query the reward function to determine the reward of this transition HINT: use self.env.get_reward
